...
from utils.data_process import tokenize_input
...
class BertWrapper(nn.Module):
	...
	def forward_per_layer(self, token_ids, segment_ids, attention_mask, pooling_type=None):
		"""
		Compute output embeddings for each layer of BERT model
		:param token_ids:
		:param segment_ids:
		:param attention_mask:
		:return:
		...
		output_bert, output_pooler, all_hidden_units, all_attention_weights = self.bert_model(token_ids, segment_ids, attention_mask)
		...
		for curr_layer_output in all_hidden_units[1:]: # Iterate over output of all layers. 0th layer is the input embedding layer so skipping it.
			if pooling_type == "cls_w_lin": # cls token embedding which is passed through linear layer in BERT model itself
				...
				# output_bert shape = (batch_size, max_seq_len, bert_output_dim)
				# output_bert[:, 0, :] -> (batch_size, bert_output_dim) tensor with values for first token in each sequence in the batch
				embeddings = curr_layer_output[:, 0, :]
			elif pooling_type == "mean":
				embeddings = torch.mean(curr_layer_output, dim=1) # Pool along dim = 1 so as to average along seq_len dimension
			elif pooling_type == "max":
				embeddings = torch.max(curr_layer_output, dim=1) # Pool along dim = 1 so as to average along seq_len dimension
			elif pooling_type == "lse": # Log-Sum-Exp pooling
				embeddings = torch.logsumexp(curr_layer_output, dim=1) # Pool along dim = 1 so as to average along seq_len dimension
			else:
				# embeddings = None
				raise NotImplementedError("Pooling type = {} not supported yet")
			
			# embeddings.shape  == (batch_size, bert_output_dim)
			assert embeddings.shape == (curr_layer_output.shape[0], self.bert_output_dim)
			
			if self.additional_linear is not None:
				# get pooled output from BERT
				# and pass it through additional layer here.
				final_embeddings = self.additional_linear(self.dropout(embeddings))
			else:
				final_embeddings = embeddings
			
			final_embeddings_list += [final_embeddings]
		
		# Each element of final_embeddings is of shape (batch_size, output_embed_dim)
		# We want output of shape : (batch_size, num_layers, output_embed_dim) so we need to add a new dim at position 1
		# and then stack them together. It is as if we first create tensors of shape (batch_size, 1, output_embed_dim)
		# and then concatenate them along dim=1 so that final_embeddings_list[i] gives (num_layers, output_embed_dim)
		# shape vector after concatenation
		final_embeddings_tensor = torch.stack(final_embeddings_list, dim=1)
		# LOGGER.info(f"Figuring out right way of batching layered output : {token_ids.shape} -> {final_embeddings_tensor.shape}")
...
class BiEncoderWrapper(LightningModule):
	...
	def forward_old(self, input_token_idxs, label_token_idxs, label_embeds=None):
		flag = label_embeds is None
		scores = self.score_labels(input_token_idxs=input_token_idxs, label_token_idxs=label_token_idxs, random_negs=flag)
		...
		if label_embeds is None:
			...
			loss = loss_fct(scores, label_embeds)
		return loss, scores
	...
	def forward_w_ent_distill(self, label_token_idxs, target_label_scores):
		"""
		Embeds labels, and computes loss for using target_labels_scores. This is used for distillation
		experiments where ONLY entity encoders are trained.
		:param label_token_idxs: Shape (batch_size, num_labels, seq_len) tensor with tokenized labels for corresponding input
		:param target_label_scores: Shape (batch_size, num_labels) tensor with scores for corresponding labels
		:return:
		"""
		...
		# # TODO: Avoid computing softmax over target label scores here?
		# target_label_scores = torch_softmax(target_label_scores)
		...
		# Reshape into original shape as per label_token_idxs
		label_embs = label_embs.view(batch_size, num_labels, -1) # Shape: (batch_size,  num_labels, embed_size)
		batch_size, num_labels, embed_dim = label_embs.shape
		
		assert self.config.loss_type in ["ce", "mse"], f"Only cross-entropy loss supported for distillation. Loss = {self.config.loss_type}"
		
		# Select label pairs
		if self.config.ent_distill_pair_method == "consec": # Pair consecutive labels together
			# First and second labels in each label pair. Consecutive labels are paired in this case
			label_pairs_0 = torch.cat([torch.arange(0, num_labels-1).unsqueeze(0) for _ in range(batch_size)]) # Shape : (batch_size, num_pairs)
			label_pairs_1 = torch.cat([torch.arange(1, num_labels).unsqueeze(0) for _ in range(batch_size)]) # Shape : (batch_size, num_pairs)
			label_pairs_0 = label_pairs_0.to(label_embs.device)
			label_pairs_1 = label_pairs_1.to(label_embs.device)
			
			num_pairs = num_labels - 1
		elif self.config.ent_distill_pair_method == "all_pairs":
			
			idxs = list(range(0, num_labels))
			all_pairs = list(itertools.combinations(idxs, 2))
			all_pairs_0, all_pairs_1 = zip(*all_pairs)
			label_pairs_0 = torch.cat([torch.LongTensor(all_pairs_0).unsqueeze(0) for _ in range(batch_size)]) # Shape : (batch_size, num_pairs)
			label_pairs_1 = torch.cat([torch.LongTensor(all_pairs_1).unsqueeze(0) for _ in range(batch_size)]) # Shape : (batch_size, num_pairs)
			label_pairs_0 = label_pairs_0.to(label_embs.device)
			label_pairs_1 = label_pairs_1.to(label_embs.device)
			num_pairs = len(all_pairs)
		# elif self.config.ent_distill_pair_method == "random":
		# 	raise NotImplementedError
		else:
			raise NotImplementedError(f"Loss not implemented for ent_distill_method = {self.config.ent_distill_pair_method }")
		
		
		# 1. First put together embeddings of first and second label in each pair
		# label_embs[i] -> shape : (num_labels, embed_dim)
		# label_pairs_0[i] -> shape: (num_pairs,)
		#
		# Following code snippet selects rows from label_embs[i] corresponding to first (or second) label in set of label pairs
		# label_embs[i][label_pairs_0[i]] -> Shape (num_pairs, embed_dim)
		# Finally, .unsqueeze(0) is used to convert shape from (num_pairs, embed_dim) to (1, num_pairs, embed_dim)
		# so that torch.cat gives a tensor of shape (batch_size, num_pairs, embed_dim) as output
		label_pairs_embeds_0 = torch.cat( [label_embs[i][label_pairs_0[i]].unsqueeze(0) for i in range(batch_size)] ) # Shape : (batch_size, num_pairs, embed_dim)
		label_pairs_embeds_1 = torch.cat( [label_embs[i][label_pairs_1[i]].unsqueeze(0) for i in range(batch_size)] ) # Shape : (batch_size, num_pairs, embed_dim)
		
		assert label_pairs_embeds_0.shape == label_pairs_embeds_1.shape
		assert label_pairs_embeds_0.shape == (batch_size, num_pairs, embed_dim),\
			f"label_pairs_embeds_0.shape = {label_pairs_embeds_0.shape} != (batch_size, num_pairs, embed_dim)={(batch_size, num_pairs, embed_dim)}"
		
		
		# 2. Compute predicted score of each pair of labels. Using dot product here.
		temp_prod = label_pairs_embeds_0*label_pairs_embeds_1 # Shape : (batch_size, num_pairs, embed_size)
		# Sum along dim = 2 which is embed_size dim to compute scores
		pred_pw_label_scores = torch.sum(temp_prod, dim=2) # Shape : (batch_size, num_pairs)
	
		# 3. Gather scores for selected first and second label in each label pair
		# target_label_scores -> shape : (batch_size, num_labels)
		# label_pairs_0 -> shape : (batch_size, num_pairs)
		tgt_label_pairs_scores_0 = torch.gather(target_label_scores, 1, label_pairs_0) # Shape : (batch_size, num_pairs)
		tgt_label_pairs_scores_1 = torch.gather(target_label_scores, 1, label_pairs_1) # Shape : (batch_size, num_pairs)
		
		assert pred_pw_label_scores.shape == (batch_size, num_pairs),\
			f"pred_pw_label_scores.shape = {pred_pw_label_scores.shape} != (batch_size, num_pairs) = {(batch_size, num_pairs)}"
		
		assert tgt_label_pairs_scores_0.shape == (batch_size, num_pairs),\
			f"tgt_label_pairs_scores_0.shape = {tgt_label_pairs_scores_0.shape} != (batch_size, num_pairs) = {(batch_size, num_pairs)}"
		
		# 4. Compute target score of paired labels. Score should be proportional to label pair similarity
		if self.config.ent_distill_pair_sim == "neg_diff":
			# Here we multiply diff w/ -1 as smaller absolute difference in target scores indicates high similarity of given pair of labels
			tgt_pw_label_scores = -1*torch.abs(tgt_label_pairs_scores_0 - tgt_label_pairs_scores_1) # Shape : (batch_size, num_pairs)
		elif self.config.ent_distill_pair_sim == "prod":
			# FIXME: This might give undue weight to pairs that have high negative scores?
			tgt_pw_label_scores = tgt_label_pairs_scores_0*tgt_label_pairs_scores_1 # Shape : (batch_size, num_pairs)
		else:
			raise NotImplementedError(f"self.config.ent_distill_pair_sim = {self.config.ent_distill_pair_sim} not supported")
			
		
		if self.config.loss_type == "ce":
			# 5. Convert target scores into probability distribution
			tgt_pw_label_scores = torch_softmax(tgt_pw_label_scores) # Shape : (batch_size, num_pairs)

			# 6. Compute loss
			loss = F.cross_entropy(input=pred_pw_label_scores, target=tgt_pw_label_scores)
		elif self.config.loss_type == "mse":
			loss = F.mse_loss(input=pred_pw_label_scores, target=tgt_pw_label_scores.to(pred_pw_label_scores.dtype))
		else:
			raise NotImplementedError(f"Loss = {self.config.loss_type} not supported")
		
		
		# # To debug vectorized implementation of pairing consecutive labels for computing loss
		# # In each input set of labels, compute score of a label with label right after it in the sequence
		# temp_prod = label_embs[:, :-1, :]*label_embs[:, 1:, :] # Shape : (batch_size, num_labels - 1, embed_size)
		# # Sum along dim = 2 which is embed_size dim to compute scores
		# pred_pw_label_scores = torch.sum(temp_prod, dim=2) # # Shape : (batch_size, num_labels - 1)
		#
		# # Compute score diff b/w two consecutive labels - Score should be proportional to label similarity
		# # Here we multiply diff w/ -1 as smaller difference indicates high similarity of labels
		# tgt_pw_label_scores = -1*torch.abs(target_label_scores[:, :-1] - target_label_scores[:, 1:])
		#
		# # Convert scores into probability distribution
		# tgt_pw_label_scores = torch_softmax(tgt_pw_label_scores)
		#
		# loss_debug =  F.cross_entropy(input=pred_pw_label_scores, target=tgt_pw_label_scores)
		# assert loss == loss_debug, f"Loss = {loss} and loss debug vals = {loss_debug} are different"
		
		return loss
	
	# def forward_w_ent_distill_w_triplets(self, label_token_idxs, target_label_scores):
	# 	"""
	# 	Sample entity triplets and train with triplet style constrastive loss
	# 	:param label_token_idxs:
	# 	:param target_label_scores:
	# 	:return:
	# 	"""
	#
	# 	batch_size, num_labels, label_seq_len = label_token_idxs.shape
	...
	def forward_w_raw(self, input_text, label_text):
		
		try:
			input_token_ids = tokenize_input(input=input_text, tokenizer=self.tokenizer, max_seq_length=self.config.max_input_len)["token_idxs"]
			label_token_ids = tokenize_input(input=label_text, tokenizer=self.tokenizer, max_seq_length=self.config.max_label_len)["token_idxs"]
			
			input_token_ids = torch.tensor(input_token_ids, dtype=torch.long).view(1, -1).to(self.device)
			label_token_ids = torch.tensor(label_token_ids, dtype=torch.long).view(1, -1).to(self.device)
			
			scores = self.score_labels(input_token_idxs=input_token_ids, label_token_idxs=label_token_ids)
			bs = scores.size(0)
	...
	def training_step(self, train_batch, batch_idx):
		if len(train_batch) == 2:
			...
			return loss
		elif len(train_batch) == 3 and self.config.neg_strategy in ["distill", "ent_distill", "trip_ent_distill", "top_ce_match"]:
			batch_input, batch_labels, batch_label_scores = train_batch
			if self.config.neg_strategy in ["distill", "top_ce_match"]:
				...
				return loss
			elif self.config.neg_strategy == "ent_distill":
				loss = self.forward_w_ent_distill(
					label_token_idxs=batch_labels,
					target_label_scores=batch_label_scores
				)
				return loss
			elif self.config.neg_strategy == "trip_ent_distill":
				loss = self.forward_w_ent_distill_w_triplets(
					label_token_idxs=batch_labels,
					target_label_scores=batch_label_scores
				)
				return loss
			else:
				raise NotImplementedError(f"Neg strategy = {self.config.neg_strategy} not implemented")
		
		elif len(train_batch) == 3 and self.config.neg_strategy in ["top_ce_w_bienc_hard_negs_ml", "top_ce_w_rand_negs_ml"]:
			raise NotImplementedError
	...
	def validation_step(self, val_batch, batch_idx):
		if len(val_batch) == 2:
			...
			loss = self.forward(batch_input, batch_pos_labels, batch_neg_labels)
			return loss
		elif len(val_batch) == 3 and self.config.neg_strategy in ["distill", "ent_distill", "top_ce_match"]:
		...
		elif len(val_batch) == 3 and self.config.neg_strategy in ["top_ce_w_bienc_hard_negs_ml", "top_ce_w_rand_negs_ml"]:
			raise NotImplementedError
	...
	def validation_epoch_end(self, outputs):
		...
		eval_loss = torch.mean(torch.tensor(outputs)) # np.mean([float(loss.cpu().numpy()) for loss in outputs])
