...
def get_index(index_path, max_nbrs, embed_type, graph_metric, entity_file, bienc_ent_embeds, ment_to_ent_scores, e2e_score_filename="", graph_type="nsw", n_long_range_frac=0.2):
	...
	if index_path is not None and os.path.isfile(index_path):
		...
		elif embed_type in ["bienc", "c-anchor", "ent", "cur-anchor"]:
...
def search_nsw_graph(nsw_graph, entity_scores, topk, beamsize, init_ents, comp_budget, masked_nodes, exit_at_local_minima_arg, pad_results):
	"""
...
def _search_nsw_graph_helper(nsw_graph, entity_scores, topk, curr_beam, explored_nodes, masked_nodes):
	"""
	...
	try:
		...
		for curr_node in curr_beam:
			...
			for curr_node_nbr in nsw_graph[curr_node]:
				if (curr_node_nbr not in explored_nodes) and (curr_node_nbr not in masked_nodes):
...
def search_hnsw_graph(hnsw_graph, entity_scores, topk, beamsize, comp_budget, masked_nodes, exit_at_local_minima_arg, pad_results):
	"""
		...
		if run_only_nsw_search: # TODO: Move if this condition in search_hnsw_graph function
			(nsw_topk_scores,
			 ...
			 nsw_curr_num_score_comps) = search_nsw_graph(
				...
				masked_nodes=masked_nodes[ment_idx],
...
def run(embed_type, res_dir, data_info, bi_model_file, graph_metric, graph_type, masked_node_frac, entry_method, entity_model_file_for_index, force_exact_init_search,
...
def main():
	...
	parser.add_argument("--embed_type", type=str, choices=["tfidf", "bienc", "anchor", "none", "c-anchor", "ent", "cur-anchor"], required=True, help="Type of embeddings to use for building NSW")
