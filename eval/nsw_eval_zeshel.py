...
def compute_ent_embeds_w_tfidf(entity_file):
	...
	# return np.array(label_embeds.todense())
...
def compute_ent_embeds(embed_type, biencoder, entity_tokens_file, entity_file):
	...
	:return: entity embeddings as dense numpy arrays
	"""
	if embed_type == "bienc" or embed_type == "ent":
...
def get_cur_embeds(train_m2e_score_filename, n_anchor_ments, test_crossenc_ment_to_ent_scores, entity_id_list, n_anchor_ents, arg_anchor_ents):
	"""
	Computes latent entity embedding using cross-encoder scores with "train" mentions.
	Latent entity embedding combines U and R matrix in CUR approximation
	Latent mention embedding is just array containing score of test mentions against anchor entities.
	If test_crossenc_ment_to_ent_scores is not given then latent mention embeddings is None
	
	:param train_m2e_score_filename:
	:param n_anchor_ments:
	:param test_crossenc_ment_to_ent_scores:
	:param entity_id_list:
	:param n_anchor_ents:
	:param arg_anchor_ents:
	:return:
	"""
	with open(train_m2e_score_filename, "rb") as fin:
		train_dump_dict = pickle.load(fin)
		train_crossenc_ment_to_ent_scores = train_dump_dict["ment_to_ent_scores"]
		train_entity_id_list = train_dump_dict["entity_id_list"]
	
	n_ents = len(train_entity_id_list)
	assert (train_entity_id_list == entity_id_list).all(), f"Entity id lists do not match"
	rng = np.random.default_rng(0)
	
	# Use last n_anchor_ments_for_mask mentions as anchor mentions
	train_crossenc_ment_to_ent_scores = train_crossenc_ment_to_ent_scores[-n_anchor_ments:, :]
	
	anchor_ents = sorted(arg_anchor_ents) if arg_anchor_ents is not None else sorted(rng.choice(n_ents, size=n_anchor_ents, replace=False))
	approx = CURApprox(
		rows=train_crossenc_ment_to_ent_scores,
		cols=train_crossenc_ment_to_ent_scores[:, anchor_ents],
		row_idxs=np.arange(train_crossenc_ment_to_ent_scores.shape[0]),
		col_idxs=anchor_ents,
		approx_preference="rows"
	)
	ent_embeds = approx.latent_cols.T
	test_ment_embeds = test_crossenc_ment_to_ent_scores[:, anchor_ents] if test_crossenc_ment_to_ent_scores is not None else None
	assert ent_embeds.shape[0] == n_ents, \
		f"ent_rep.shape[0] = {ent_embeds_for_masking.shape[0]} != n_ents = {n_ents}"
	
	ent_embeds = np.ascontiguousarray(ent_embeds.cpu().numpy())
	test_ment_embeds = np.ascontiguousarray(test_ment_embeds.cpu().numpy()) if test_ment_embeds is not None else None
	
	return ent_embeds, test_ment_embeds


def get_approx_scores_and_masked_nodes(train_m2e_score_filename, entity_id_list, masked_node_frac, masking_type, arg_anchor_ents_and_scores):
	"""
	Compute approximate score for all entities using scores for a set of anchor entities in arg_anchor_ents_and_scores
	:param train_m2e_score_filename:
	:param entity_id_list:
	:param masked_node_frac:
	:param masking_type:
	:param arg_anchor_ents_and_scores: Dictionary containing entity index as key and and score as value
	:return:
	"""

	if masking_type == "adapt_soft":
		# Soft masking will avoid computing exact scores for these nodes and will use approximate scores instead
		# These scores will get updated as we compute exact cross-encoder scores for more and more nodes during search
		
		
		# If number of entities scored is less than a threshold then do not use CUR approximation.
		if len(arg_anchor_ents_and_scores) < 50:
			return None, {}
		
		if masked_node_frac == 0.0:
			return None, {}
		
		# First extract ids for entities that have been already scored, and also extract the scores of these entities
		anchor_ents = np.array(sorted(arg_anchor_ents_and_scores.keys()))
		anchor_ent_scores = np.array([arg_anchor_ents_and_scores[_n] for _n in anchor_ents])
		
		ent_embeds, _ = get_cur_embeds(
			train_m2e_score_filename=train_m2e_score_filename,
			entity_id_list=entity_id_list,
			n_anchor_ents=None,
			arg_anchor_ents=anchor_ents,
			n_anchor_ments=0,
			test_crossenc_ment_to_ent_scores=None,
		)
		temp_ment_rep = anchor_ent_scores # Shape: len(arg_anchor_ents)
		temp_scores = torch.tensor(np.matmul(temp_ment_rep, ent_embeds.T)) # len(arg_anchor_ents) x ( len(arg_anchor_ents), num_ents)
		
		n_ents = len(entity_id_list)
		
		assert 0 < masked_node_frac < 1, f"masked_node_frac = {masked_node_frac} should be b/w 0 and 1"
		nodes_to_search = int( (1 - masked_node_frac)*n_ents )
		
		_, top_nodes = temp_scores.topk(nodes_to_search)
		masked_nodes = set(range(n_ents)) - set(top_nodes.cpu().numpy())
		temp_scores = temp_scores.cpu().numpy()
		
		return temp_scores, masked_nodes
	
	else:
		raise NotImplementedError(f"masking_type = {masking_type} not supported in get_masked_nodes_and_approx_scores function")
...
def get_index(index_path, max_nbrs, embed_type, graph_metric, entity_file, bienc_ent_embeds, ment_to_ent_scores, e2e_score_filename="", graph_type="nsw", n_long_range_frac=0.2):
	...
	if index_path is not None and os.path.isfile(index_path):
		...
		elif embed_type in ["bienc", "c-anchor", "ent", "cur-anchor", "cur"]:
...
def get_init_ents(init_ent_method, k, n_ments, n_ents, ment_embeds, ent_embeds, force_exact_search):
	...
	if init_ent_method == "random" or init_ent_method == "anchor":
		...
		return init_ents
	elif init_ent_method in ["bienc", "tfidf", "cur"]:
...
def search_nsw_graph(nsw_graph, entity_scores, approx_entity_scores_and_masked_nodes, topk, arg_beamsize, init_ents, comp_budget, exit_at_local_minima_arg, pad_results):
	"""
	...
	:param approx_entity_scores_and_masked_nodes: Tuple containing (a,b) or a function that return (a,b) where
		a = Approx score for each entity node in the graph or a function to compute these scores
		b = Set of nodes that are masked out and are ignored by greedy graph search
	:param topk: Number of top scoring entities to find in the graph
	:param arg_beamsize: Size of beam for greedy search over NSW graph
	...
	try:
		...
		# Needed to strictly enforce comp budget. If comp_budget > 0 then strictly enforce the budget.
		# If comp_budget = 0 then allow for one-time violation of comp_budget when choosing initial beam
		# as this allows us to also evaluate initial entry point selection method
		if (comp_budget is None) or (comp_budget == 0):
			beamsize = arg_beamsize
		else:
			beamsize = min(arg_beamsize, comp_budget)
		...
		explored_nodes_exact = copy.deepcopy(explored_nodes)
		num_score_comps += len(explored_nodes)
		
		topk_nodes_n_scores = _get_topk(curr_node_n_score_tuples=explored_nodes.items(), topk=topk)
		
		# TODO: Implement masking for entry points as well? Currently masking of nodes does not affect entry points in the graph
		
		
		if isinstance(approx_entity_scores_and_masked_nodes, types.FunctionType):
			approx_entity_scores, masked_nodes = approx_entity_scores_and_masked_nodes(explored_nodes_exact)
		else:
			approx_entity_scores, masked_nodes = approx_entity_scores_and_masked_nodes
		
		shortlist_alpha =  2 # TODO: Get this as argument
		for search_iter in range(n_ents): # This is maximum number of search iterations we need
			...
			if (comp_budget is not None) and num_score_comps >= comp_budget: # Equal sign to strictly enforce comp_budget constraint
				break
			
			# Update masked_nodes and approx_entity_scores array if approx_entity_scores_and_masked_nodes is a function
			if isinstance(approx_entity_scores_and_masked_nodes, types.FunctionType) and len(explored_nodes_exact) > curr_explored_thresh:
				approx_entity_scores, masked_nodes = approx_entity_scores_and_masked_nodes(explored_nodes_exact)
				curr_explored_thresh *= 2
				
			# # Find best-nodes amongst nbrs of nodes in current beam
			# curr_topk_nodes_n_scores, curr_num_score_comps = _search_nsw_graph_helper(
			# 	nsw_graph=nsw_graph,
			# 	entity_scores=entity_scores,
			# 	approx_entity_scores=approx_entity_scores,
			# 	topk=max(topk, beamsize), # curr_topk_nodes_n_scores is used for updating beam and topk entities found so far, so it should have max of (topk, beamsize) elements.
			# 	curr_beam=curr_beam,
			# 	explored_nodes=explored_nodes,
			# 	explored_nodes_exact=explored_nodes_exact,
			# 	masked_nodes=masked_nodes,
			# 	local_comp_budget=comp_budget - num_score_comps if comp_budget is not None else 999999999999999999, # Pass remaining computation budget
			# )
			
			# Find best-nodes amongst nbrs of nodes in current beam
			curr_topk_nodes_n_scores, curr_num_score_comps = _search_nsw_graph_helper_local_approx(
				nsw_graph=nsw_graph,
				entity_scores=entity_scores,
				approx_entity_scores=approx_entity_scores,
				topk=max(topk, beamsize), # curr_topk_nodes_n_scores is used for updating beam and topk entities found so far, so it should have max of (topk, beamsize) elements.
				...
				explored_nodes=explored_nodes,
				explored_nodes_exact=explored_nodes_exact,
				shortlist_alpha=shortlist_alpha,
				local_comp_budget=comp_budget - num_score_comps if comp_budget is not None else 999999999999999999, # Pass remaining computation budget
			)
			
			num_score_comps += curr_num_score_comps
			
			# LOGGER.info(f"Search_iter:{search_iter}, curr_num_score_comps:{curr_num_score_comps}, num_score_comps:{num_score_comps}, beamsize={beamsize}, nsw_graph:{len(nsw_graph[0])}")
			...
			if len(curr_topk_nodes_n_scores) == 0:
				# If all nbrs of nodes in beam are explored i.e, curr_topk_nodes_n_scores is empty
				# This means that we have hit a dead end but not necessarily a local minima
				break
				
			if len(topk_nodes_n_scores) >= topk and curr_topk_nodes_n_scores[0][1] < topk_nodes_n_scores[-1][1] :
				if exit_at_local_minima or (num_score_comps >= comp_budget):
					break
		...
		assert (comp_budget is None) or (comp_budget == 0) or (num_score_comps <= comp_budget), f"We have exceed the comp budget = {comp_budget} by using {num_score_comps} calls to cross-encoder"
		...
		# assert num_score_comps == len(explored_nodes), f"num_score_comps = {num_score_comps} != len(explored_nodes) = {len(explored_nodes)}"
		# adjusted_num_score_comps = len({n for n in explored_nodes if n not in masked_nodes})
		#
		# return np.array(topk_scores), np.array(topk_nodes), adjusted_num_score_comps
	except Exception as e:
		embed()
		raise e


def _search_nsw_graph_helper_old(nsw_graph, entity_scores, topk, curr_beam, explored_nodes, masked_nodes, local_comp_budget):
	"""
	...
	:param masked_nodes: Set of nodes that are masked out and are ignored by greedy graph search
	:return:
	"""
	try:
		num_score_comps = 0
		...
		for curr_node in curr_beam:
			# Iterate over all unexplored neighbors of this node which are not masked and score them
			for curr_node_nbr in nsw_graph[curr_node]:
				if (curr_node_nbr not in explored_nodes) and (curr_node_nbr not in masked_nodes) and (len(nodes_to_visit) < local_comp_budget): # Last clause in if condition to strictly enforce budget constraints
...
def _search_nsw_graph_helper(nsw_graph, entity_scores, approx_entity_scores, topk, curr_beam, explored_nodes, explored_nodes_exact, masked_nodes, local_comp_budget):
	"""
	Searches over nbrs of nodes in beam and return topk neighbors that are not part of explored_nodes
	Modifies explored_nodes and explored_nodes_exact in place.
	:param nsw_graph: NSW graph to search over
	:param entity_scores: Score for each entity node in the graph or a function to compute these scores
	:param topk: Number of top scoring entities to find in the graph
	:param curr_beam: List of entities in current beam during search over NSW
	...
	try:
		num_score_comps = 0
		curr_node_n_score_tuples = []
		nodes_to_visit_exact  = []
		nodes_to_visit_approx  = []
		for curr_node in curr_beam:
			# Iterate over all unexplored neighbors of this node and score them
			for curr_node_nbr in nsw_graph[curr_node]:
				if curr_node_nbr not in explored_nodes:
					if curr_node_nbr not in masked_nodes: # Compute exact score for this node
						if len(nodes_to_visit_exact) < local_comp_budget: # to strictly enforce budget constraints
							nodes_to_visit_exact += [curr_node_nbr]
							explored_nodes[curr_node_nbr] = -1
							explored_nodes_exact[curr_node_nbr] = -1
					else: # Approximate score for this node
						nodes_to_visit_approx += [curr_node_nbr]
						explored_nodes[curr_node_nbr] = -1
				
	
		# Score nodes in nodes_to_visit_exact wrt given mention using exact method
		if isinstance(entity_scores, np.ndarray):
			nodes_to_visit_scores = entity_scores[nodes_to_visit_exact]
		elif isinstance(entity_scores, types.FunctionType):
			nodes_to_visit_scores = entity_scores(nodes_to_visit_exact)
		else:
			raise Exception(f"Entity scores of type = {type(entity_scores)} not supported")
		
		# Score nodes in nodes_to_visit_approx wrt given mention using approx method
		if isinstance(approx_entity_scores, np.ndarray):
			nodes_to_visit_scores = np.concatenate((nodes_to_visit_scores, approx_entity_scores[nodes_to_visit_approx]))
		elif approx_entity_scores is None:
			pass # Do not score these nodes at all, just ignore them
		else:
			raise Exception(f"Entity scores of type = {type(approx_entity_scores)} not supported")
		
		
		if approx_entity_scores is None:
			assert len(nodes_to_visit_scores) == len(nodes_to_visit_exact), f"Len of nodes_to_visit_scores ={len(nodes_to_visit_scores)} != len of nodes_to_visit = {len(nodes_to_visit_exact)} when approx_entity_scores = {approx_entity_scores}"
		else:
			assert len(nodes_to_visit_scores) == len(nodes_to_visit_exact) + len(nodes_to_visit_approx), f"Len of nodes_to_visit_scores ={len(nodes_to_visit_scores)} != len of nodes_to_visit = {len(nodes_to_visit_exact) + len(nodes_to_visit_approx)} when approx_entity_scores = {approx_entity_scores}"
		
		num_score_comps += len(nodes_to_visit_exact) # Only count nodes for which exact score was computed
		
		# zip will ignore nodes_to_visit_approx in case approx_entity_scores is None and we have not added any approximate scores in nodes_to_visit_scores
		curr_node_n_score_tuples += list(zip(nodes_to_visit_exact + nodes_to_visit_approx, nodes_to_visit_scores))
		explored_nodes.update({node:score for node, score in curr_node_n_score_tuples})
		# Update exact scores
		explored_nodes_exact.update({node:score for node, score in zip(nodes_to_visit_exact, nodes_to_visit_scores)})
...
def _search_nsw_graph_helper_local_approx(nsw_graph, entity_scores, approx_entity_scores, topk, curr_beam, explored_nodes, explored_nodes_exact, shortlist_alpha, local_comp_budget):
	"""
	Searches over nbrs of nodes in beam and return topk neighbors that are not part of explored_nodes
	Modifies explored_nodes and explored_nodes_exact in place.
	:param nsw_graph: NSW graph to search over
	...
	:param curr_beam: List of entities in current beam during search over NSW
	:param explored_nodes: List of entities that have been explored during search
	:param shortlist_alpha: int > 1. This controls how many nodes are shortlisted for computing exact scores
	:return:
	"""
	try:
		nodes_to_visit  = []
		...
		# First approximate scores for all neighbors
		if isinstance(approx_entity_scores, np.ndarray):
			nodes_to_visit_approx_scores = approx_entity_scores[nodes_to_visit]
		elif approx_entity_scores is None:
			pass # Do not score these nodes at all, just ignore them
			nodes_to_visit_approx_scores = np.ones((len(nodes_to_visit),))
		else:
			raise Exception(f"Entity scores of type = {type(approx_entity_scores)} not supported")
		
		
		# TODO: Also allow for threshold based picking of nodes which need to scored exactly
		# Now pick top-k nodes using approx scores and compute exact score for them
		sorted_nodes_and_approx_scores = sorted(zip(nodes_to_visit, nodes_to_visit_approx_scores), reverse=True, key=lambda x:x[1])
		n_nodes_for_exact_score = (shortlist_alpha*topk)
		if len(sorted_nodes_and_approx_scores) > 0:
			nodes_to_visit_exact, _ = zip(*sorted_nodes_and_approx_scores[:n_nodes_for_exact_score])
			nodes_to_visit_exact = np.array(nodes_to_visit)
		else:
			nodes_to_visit_exact = []
			
		# Enforce strict budget constraint
		nodes_to_visit_exact = nodes_to_visit_exact[:local_comp_budget]
		
		# Score nodes in nodes_to_visit_exact wrt given mention using exact method
		if isinstance(entity_scores, np.ndarray):
			nodes_to_visit_exact_scores = entity_scores[nodes_to_visit_exact]
		elif isinstance(entity_scores, types.FunctionType):
			nodes_to_visit_exact_scores = entity_scores(nodes_to_visit_exact)
		else:
			raise Exception(f"Entity scores of type = {type(entity_scores)} not supported")
		
		# TODO: Add these assert statement back again
		# if approx_entity_scores is None:
		# 	assert len(nodes_to_visit_scores) == len(nodes_to_visit_exact), f"Len of nodes_to_visit_scores ={len(nodes_to_visit_scores)} != len of nodes_to_visit = {len(nodes_to_visit_exact)} when approx_entity_scores = {approx_entity_scores}"
		# else:
		# 	assert len(nodes_to_visit_scores) == len(nodes_to_visit_exact) + len(nodes_to_visit_approx), f"Len of nodes_to_visit_scores ={len(nodes_to_visit_scores)} != len of nodes_to_visit = {len(nodes_to_visit_exact) + len(nodes_to_visit_approx)} when approx_entity_scores = {approx_entity_scores}"
		...
		# Update nodes explored with approx scores
		explored_nodes.update({node:score for node, score in zip(nodes_to_visit, nodes_to_visit_approx_scores)})
		
		# Update nodes explored with exact scores
		explored_nodes_exact.update({node:score for node, score in zip(nodes_to_visit_exact, nodes_to_visit_exact_scores)})
		
		# Pick top-k nodes to put in beam and search again
		nodes_and_exact_scores = zip(nodes_to_visit_exact, nodes_to_visit_exact_scores)
		topk_nodes_n_scores = _get_topk(curr_node_n_score_tuples=nodes_and_exact_scores, topk=topk)
		return topk_nodes_n_scores, num_score_comps
...
def search_nsw_graph_w_backtrack(nsw_graph, entity_scores, topk, beamsize, init_ents, comp_budget, exit_at_local_minima_arg, pad_results, masked_nodes):
	"""
	...
	try:
		...
		for search_iter in range(n_ents): # This is maximum number of search iterations we need
			...
			) = _search_nsw_graph_w_backtrack_helper(
				nsw_graph=nsw_graph,
				entity_scores=entity_scores,
				topk=topk,
				curr_beam=curr_beam,
...
def search_hnsw_graph(hnsw_graph, entity_scores, approx_entity_scores_and_masked_nodes, topk, beamsize, comp_budget, exit_at_local_minima_arg, pad_results):
	"""
	...
	:param pad_results
	:param approx_entity_scores_and_masked_nodes
	:return:
	"""
	...
	for curr_depth, curr_nsw_graph in enumerate(hnsw_graph):
		...
		(nsw_topk_scores,
		 ...
		 nsw_curr_num_score_comps) = search_nsw_graph(
			entity_scores=entity_scores,
			approx_entity_scores_and_masked_nodes=approx_entity_scores_and_masked_nodes,
			nsw_graph=curr_nsw_graph,
			topk=curr_topk,
			arg_beamsize=curr_beamsize,
...
def run_hnsw_search(
	...
	:param masked_nodes: Dict mapping mention_idx to set of nodes masked/to-be-ignored for the corresponding mentions
	...
	for ment_idx in range(n_ments):
		...
		approx_entity_scores = approx_ment_to_ent_scores if isinstance(approx_ment_to_ent_scores, types.FunctionType) else approx_ment_to_ent_scores[ment_idx]
		...
		if run_only_nsw_search: # TODO: Move if this condition in search_hnsw_graph function
			(nsw_topk_scores,
			 ...
			 nsw_curr_num_score_comps) = search_nsw_graph(
				entity_scores=entity_scores,
				approx_entity_scores_and_masked_nodes=approx_entity_scores if isinstance(approx_entity_scores, types.FunctionType) else (approx_entity_scores, masked_nodes[ment_idx]),
...
def run(embed_type, res_dir, data_info, bi_model_file, graph_metric, graph_type,
		masking_type, masked_node_method, masked_node_frac,
		train_m2e_score_filename, entry_method, entity_model_file_for_index, force_exact_init_search,
	...
	try:
		...
		if os.path.isfile(entity_model_file_for_index):
			try:
				entity_model_for_index = BiEncoderWrapper.load_from_checkpoint(entity_model_file_for_index)
			except:
				LOGGER.info("Loading entry_bi_model_file biencoder trained with shared params with a cross-encoder")
				entity_model_for_index = CrossEncoderWrapper.load_from_checkpoint(entity_model_file_for_index)
			entity_model_for_index.eval()
		else:
			entity_model_for_index = None
		
		
		
		# Compute mention and entity embeddings to use for choosing entry points in graph
		if entry_method ==  "cur":
			ent_embeds_for_init, ment_embeds_for_init = get_cur_embeds(
				test_crossenc_ment_to_ent_scores=crossenc_ment_to_ent_scores,
				train_m2e_score_filename=train_m2e_score_filename,
				n_anchor_ents=100,
				n_anchor_ments=500,
				entity_id_list=entity_id_list,
				arg_anchor_ents=None
			)
			
		else:
			# 1.)  Compute mention embeddings to use for choosing entry points in graph
			LOGGER.info(f"Computing mention encodings using method = {entry_method}")
			ment_embeds_for_init = compute_ment_embeds(
			...
			LOGGER.info(f"Computing entity encodings using method = {entry_method}")
		...
		LOGGER.info(f"Computing entity encodings for indexing using method = {embed_type}")
		if (embed_type == "bienc") and (embed_type == entry_method):
			...
			ent_embeds_for_index = compute_ent_embeds(
				embed_type=embed_type,
				biencoder=entity_model_for_index,
				entity_tokens_file=data_fname["ent_tokens_file"],
				entity_file=data_fname["ent_file"],
			)
			...
			# Heuristic to only keep mentions whose gt entity is an anchor entity
			# new_gt_labels = []
			...
			pass
		elif embed_type == "cur-anchor":
			# Use CUR like method on top of anchor embeddings for entities.
			# These anchor embeds are computed using score against test mentions
			n_anchor_ments = 100
			anchor_ments = np.arange(n_anchor_ments)
			n_anchor_ents = 500
			rng = np.random.default_rng(seed=0)
			anchor_ents = rng.choice(n_ents, size=n_anchor_ents, replace=False)
			
			
			R = crossenc_ment_to_ent_scores[anchor_ments,:] # shape:(n_anchor_ments, num_entities)
			U = R[:, anchor_ents] # shape: n_anchor_ments, n_anchor_ents
			U = torch.tensor(np.linalg.pinv(U)) # shape: n_anchor_ents, n_anchor_ments
			ent_embeds_for_index = U @ R # shape: n_anchor_ents, num_entities
			ent_embeds_for_index  = ent_embeds_for_index.T
			ent_embeds_for_index  = ent_embeds_for_index.cpu().detach().numpy()
			ment_idxs_to_use = np.arange(n_anchor_ments, 200) # FIXME: Use all available mentions
			curr_gt_labels = curr_gt_labels[n_anchor_ments:200]
		elif embed_type == "cur":
			# Use CUR like method on top of anchor embeddings for entities.
			# These anchor embeds are computed using score against separate set of mentions (different from test mentions)
			ent_embeds_for_index, _ = get_cur_embeds(
				test_crossenc_ment_to_ent_scores=crossenc_ment_to_ent_scores,
				train_m2e_score_filename=train_m2e_score_filename,
				n_anchor_ents=100,
				n_anchor_ments=500,
		...
		init_ents = get_init_ents(
			...
			k=max(beamsize_vals),
			ment_embeds=ment_embeds_for_init,
			ent_embeds=ent_embeds_for_init,
			n_ments=n_ments,
			n_ents=n_ents,
			force_exact_search=force_exact_init_search
		)
		if masked_node_frac > 0:
			
			if masked_node_method == "cur": # Use CUR method for computing entity and mention embeddings for masking
				ent_embeds_for_masking, ment_embeds_for_masking = get_cur_embeds(
					test_crossenc_ment_to_ent_scores=crossenc_ment_to_ent_scores,
					train_m2e_score_filename=train_m2e_score_filename,
					n_anchor_ents=100,
					n_anchor_ments=500,
			...
			else: # use embeddings used for finding initial entry points for
				# 1.)  Compute mention embeddings to use for choosing entry points in graph
				LOGGER.info(f"Computing mention encodings using method = {entry_method}")
				ment_embeds_for_masking = compute_ment_embeds(
					embed_type=masked_node_method,
					entity_file=data_fname["ent_file"],
					mentions=mentions,
					biencoder=biencoder,
					mention_tokens_list=mention_tokens_list
				...
				# 2.)  Compute entity embeddings to use for choosing entry points in graph
				LOGGER.info(f"Computing entity encodings using method = {entry_method}")
				ent_embeds_for_masking = compute_ent_embeds(
					embed_type=masked_node_method,
					biencoder=biencoder,
					entity_tokens_file=data_fname["ent_tokens_file"],
					entity_file=data_fname["ent_file"],
			...
			top_nodes = get_init_ents(
				init_ent_method=masked_node_method,
				k=nodes_to_search,
				ment_embeds=ment_embeds_for_masking,
				ent_embeds=ent_embeds_for_masking,
				n_ments=n_ments,
				n_ents=n_ents,
				force_exact_search=True
			)
			all_nodes_set = set(range(n_ents))
			masked_nodes = {ment_idx:all_nodes_set - set(top_nodes[ment_idx]) for ment_idx in range(n_ments)}
			
			if masking_type == "hard": # Hard masking will make sure that these nodes are never part of the (beam-)search
				approx_crossenc_ment_to_ent_scores = {_x:None for _x in range(n_ments)}
			elif masking_type == "soft":
				# Soft masking will avoid computing exact scores for these nodes and will use approximate scores instead
				approx_crossenc_ment_to_ent_scores = np.matmul(ment_embeds_for_masking, ent_embeds_for_masking.T)
			elif masking_type == "adapt_soft":
				approx_crossenc_ment_to_ent_scores = lambda arg_anchor_ents_and_scores: get_approx_scores_and_masked_nodes(
					entity_id_list=entity_id_list,
					train_m2e_score_filename=train_m2e_score_filename,
					masked_node_frac=masked_node_frac,
					masking_type=masking_type,
					arg_anchor_ents_and_scores=arg_anchor_ents_and_scores
				)
				# No nodes will be masked upfront. We will use scores computed during score graph search to mask nodes
				masked_nodes = {ment_idx:{} for ment_idx in range(n_ments)}
			else:
				raise NotImplementedError(f"masking_type = {masking_type} not supported")
		...
		for max_nbr_ctr, max_nbrs in enumerate(max_nbr_vals):
			...
			iter_vals = list(itertools.product(budget_vals, topk_vals, beamsize_vals))
			for comp_budget, topk, beamsize in tqdm(iter_vals):
				key = f"k={topk}_b={beamsize}_init_{entry_method}_budget={comp_budget}_max_nbrs={max_nbrs}"
				result[key] = {}
				...
				crossenc_result = run_hnsw_search(
					...
					ment_to_ent_scores=crossenc_ment_to_ent_scores,
					approx_ment_to_ent_scores=approx_crossenc_ment_to_ent_scores,
...
def main():
	...
	parser.add_argument("--embed_type", type=str, choices=["tfidf", "bienc", "anchor", "none", "c-anchor", "ent", "cur-anchor", "cur"], required=True, help="Type of embeddings to use for building NSW")
	
	
	parser.add_argument("--bi_model_file", type=str, default="", help="Biencoder Model config file or ckpt file")
	parser.add_argument("--entity_model_file_for_index", type=str, default="", help="Model config file or ckpt file for embedding entities for indexing")
	...
	parser.add_argument("--train_m2e_score_filename", type=str, default="", help="Pickle file containing mention-entity scores information (this is used to get entity embeddings for indexing entities using CUR method. This contains scores for mentions other than test mentions)")
	...
	parser.add_argument("--entry_method", type=str, default="bienc", choices=["bienc", "tfidf", "random", "cur"], help="Method for choosing entry points in graph")
	parser.add_argument("--masking_type", type=str, default="soft", choices=["soft", "hard", "adapt_soft"], help="Hard masking completely ignores masked nodes, soft masking avoid calling exact scoring function and uses approx score instead for masked nodes during search process")
	parser.add_argument("--masked_node_method", type=str, default="bienc", choices=["bienc", "tfidf", "cur"], help="Method for choosing which nodes to mask in graph during search")
	parser.add_argument("--masked_node_frac", type=float, default=0.0, help="Fraction to top-nodes to consider while searching. Other nodes will be masked out and ignored during search")
	...
	entity_model_file_for_index = args.entity_model_file_for_index
	...
	masked_node_frac = args.masked_node_frac
	masked_node_method = args.masked_node_method
	...
	train_m2e_score_filename = args.train_m2e_score_filename
	...
	assert embed_type != "ent" or (entity_model_file_for_index is not None and os.path.isfile(entity_model_file_for_index)) , f"entity_model_file_for_index = {entity_model_file_for_index} should be valid filepath if embed_type = ent"
	...
	if False and debug_mode:
		...
		# topk_vals = [64]
		# beamsize_vals = [5, 10]
		# budget_vals = [None, 500]
	else:
		max_nbr_vals = [5, 10, 20, 50]
		# topk_vals = [1, 10, 64, 100, 250, 500, 1000]
		# beamsize_vals = [1, 2, 5, 10, 50, 64, 100, 250, 500, 1000]
		# budget_vals = [None, 0, 64, 100, 250, 500, 1000, 2000]
		topk_vals = [1, 10, 64, 100]
		beamsize_vals = [1, 2, 5, 10, 64, 100, 250, 500, 1000]
		budget_vals = [0, 64, 100, 250, 500, 1000, 2000]
	...
	for world_type, world_name in tqdm(iter_worlds):
		...
		if not plot_only:
			run(
				...
				entity_model_file_for_index=entity_model_file_for_index,
				graph_metric=graph_metric,
				graph_type=graph_type,
