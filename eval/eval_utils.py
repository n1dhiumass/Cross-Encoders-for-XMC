...
def compute_recall(gt_indices_list, pred_indices_list):
	return compute_overlap(
		indices_list1=gt_indices_list,
		indices_list2=pred_indices_list,
		allow_diff_size_lists=True,
		allow_multi_sets=True
	)


def compute_overlap(indices_list1, indices_list2, allow_diff_size_lists=False, allow_multi_sets=True):
	"""
	Comute overlap metrics b/w corresponding pairs of lists
	:param indices_list1:
	:param indices_list2:
	:param allow_multi_sets: If FALSE, then error is thrown for when lists contain duplicates
	:return:
	"""
	all_res = []
	for indices1, indices2 in zip(indices_list1, indices_list2):
		res = _compute_overlap_helper(
			indices1=indices1,
			indices2=indices2,
			allow_diff_size_lists=allow_diff_size_lists,
			allow_multi_sets=allow_multi_sets
		)
		all_res += [res]
...
def _compute_overlap_helper(indices1, indices2, allow_multi_sets=True, allow_diff_size_lists=False):
	n_intersection = len(set(indices1).intersection(set(indices2)))
	n_total = len(indices1) + len(indices2)
	
	assert allow_diff_size_lists or (len(indices1) == len(indices2)), f"Len of both indices is not same => {len(indices1)} != {len(indices2)} and this function is not symmetric wrt indices1 and indices2"
	if allow_diff_size_lists:
		warnings.warn("This function is not symmetric in this arguments!!!! Double check before using it")
		
	if not allow_multi_sets:
		assert len(indices1) == len(set(indices1)), f"Duplicate in indices1 as len of => len(indices1) = {len(indices1)} != len(set(indices1)) = {len(set(indices1))}"
		assert len(indices2) == len(set(indices2)), f"Duplicate in indices1 as len of => len(indices2) = {len(indices2)} != len(set(indices2)) = {len(set(indices2))}"
