...
def _get_eval_file_from_dir(dir_name, ckpt_type, eval_data, graph_config):
	...
	elif dir_name.startswith("bm25"):
		eval_file = f"{dir_name}/method=bm25_d={eval_data}.json"
	elif dir_name.startswith("graph"):
		eval_file = f"{dir_name}/{graph_config}/method=graph_d={eval_data}.json"
	else:
	...
	try:
		...
		if "model=CUR" in row_name:
			...
			new_row_name = "~".join([param_n_val if param_n_val.split("=")[0] in relevant_params["TFIDF"]
									 ...
									 for param_n_val in split_row_name_list ])
		elif "BM25" in row_name:
			new_row_name = "~".join([param_n_val if param_n_val.split("=")[0] in relevant_params["BM25"]
									 else f"{param_n_val.split('=')[0]}=None"
									 for param_n_val in split_row_name_list ])
			...
			new_row_name = "~".join([param_n_val if param_n_val.split("=")[0] in relevant_params["Fixed_Anc_Ent_CUR"]
									 ...
									 for param_n_val in split_row_name_list ])
		elif "graph" in row_name:
			new_row_name = "~".join([param_n_val if param_n_val.split("=")[0] in relevant_params["graph"]
									 else f"{param_n_val.split('=')[0]}=None"
									 for param_n_val in split_row_name_list ])
		else:
			...
			for row_val in row_keys: # For each row, get value of all cols i.e. for combinations of x-axis-param-vals
				...
				if trimmed_row_val in added_row_vals: # Max over all rows that get trimmed to this trimmed_row_val
					...
					try:
						...
						row_dict = {k:v for k,v in row_dict.items() if k in col_keys}
					...
					try:
						...
						max_row_dict = {k: [v] + prev_row_dict[k] for k,v in row_dict.items() if k in col_keys}
...
def process_res_for_rq(combined_res, template, all_param_vals, fixed_params, var_params, x_axis_params, val_type, same_cost_opt):
	...
	final_res_cost_split = defaultdict(lambda :defaultdict(dict))
	...
	for curr_fixed_param_vals in itertools.product(*all_fixed_param_vals):
		...
		for curr_var_param_vals in itertools.product(*all_var_param_vals):
			...
			for curr_x_axis_param_vals in itertools.product(*all_x_axis_param_vals):
				cost_split = (curr_x_axis_param_vals[0], 0)
				if same_cost_opt:
					if "CUR" in var_key: # For CUR add top_k_retvr and n_ent_anchor vals
						# x_axis_key = f"cost={curr_x_axis_param_vals[0] + curr_x_axis_param_vals[1]}@" + "~".join([ f"{param}={param_val}" for param, param_val in zip(x_axis_params, curr_x_axis_param_vals)])
						x_axis_key = f"cost={curr_x_axis_param_vals[0] + curr_x_axis_param_vals[1]}"
						cost_split = (curr_x_axis_param_vals[0], curr_x_axis_param_vals[1])
						...
						# x_axis_key = f"cost={curr_x_axis_param_vals[0] +  nm_train_val}"
						x_axis_key = f"cost={curr_x_axis_param_vals[0]}"
					else: # For other methods just use top_k_retvr vals
						# x_axis_key = f"cost={curr_x_axis_param_vals[0]}@" + "~".join([ f"{param}={param_val}" for param, param_val in zip(x_axis_params, curr_x_axis_param_vals)])
						x_axis_key = f"cost={curr_x_axis_param_vals[0]}"
				else:
				...
				if x_axis_key in final_res[fixed_key][var_key]: # Take best possible value if there are multiple values for this x_axis_key
					if final_res[fixed_key][var_key][x_axis_key] < combined_res[comb_key][val_type]:
						final_res_cost_split[fixed_key][var_key][x_axis_key] = cost_split
	...
	return final_res, final_res_cost_split
...
def run(data_name, base_res_dir):
	...
	model_vals = {
		...
		# "02_D_BE_MATCH_25"	: "m=bi_enc_l=ce_neg=top_ce_match_s=1234_topk_25_from_6_20_bienc",
		...
		# "02_S_BE_MATCH_25"	: "m=bi_enc_l=ce_neg=top_ce_match_s=1234_topk_25_from_scratch",
	...
	# Graph-search-params
	embed_type_vals  = ["bienc", "tfidf"]
	max_nbrs_vals  = [5, 10, 20, 50]
	beamsize_vals  = [1, 2, 5, 10]
	# entry_method_vals  = ["random", "bienc"]
	entry_method_vals  = ["random"]
	...
	graph_config_vals = [""]
	# graph_config_vals = [
	# 	f"emb={emb}_nbr={max_nbr}_b={beamsize}_entry={entry}"
	# 	for emb, max_nbr, beamsize, entry in itertools.product(embed_type_vals, max_nbrs_vals, beamsize_vals, entry_method_vals)
	# ]
	...
	"var_params": ["model", "ckpt"],
	"fixed_params": ["nm_train", "top_k", "data_type"],
	"x_axis_params": ["top_k_retvr", "anc_n_e"],
	"val_type": "prec@k",
	"same_cost_opt": True
	
	- Comparison of methods for same k-retrieved
	3 splits and 5 datasets, 4 top-k vals = 1,10,50,100
	for 500, yugioh, top-100 and top-1 - add as table and add remaining in appendix
	
	
	- Comparison of methods for same cost
	3 splits and 5 datasets, 4 top-k vals
	for 500, yugioh, top-100 and top-1 - add as table and add remaining in appendix
	
	- How does performance change as we increase number of anchor mentions - this is
	about increasing size of training data.
	for yugioh
	- Also record training time and time to compute mention-entity score matrix
	
	- How does performance vary as we increase number of entities - dataset specific
	
	- Performance difference b/w original test data and train data -
	compare yugioh vs pro and star-trek vs doctor-who esp for top-1?
	
	
	
	
	"var_params": ["model", "ckpt", "anc_n_e"],
	"fixed_params": ["split_idx", "top_k", "data_type", "top_k_retvr"],
	"x_axis_params": ["nm_train"],
	"val_type": "prec@k",
	"same_cost_opt": False
	...
	for curr_rq in RQs:
		...
		processed_res, processed_res_cost_split = process_res_for_rq(
			template=template,
		...
		process_fname_w_cost_split = f"{curr_rq_res_dir}/processed_res_cost_split.json"
		Path(os.path.dirname(process_fname_w_cost_split)).mkdir(exist_ok=True, parents=True)
		with open(process_fname_w_cost_split, "w") as fout:
			json.dump(processed_res_cost_split, fout, indent=4)
		...
		# Plot data or convert into table
		plot_processed_results(
			processed_res_fname=process_fname_w_cost_split,
			res_dir=f"{curr_rq_res_dir}/plots_w_cost_split",
			# all_param_vals=all_param_vals if (not RQs[curr_rq]["same_cost_opt"]) else all_param_vals_same_cost, # Need to use all_param_vals_same_cost here as we only want to look at a table with just top_k_retvr_values = top_k_retr_vals_base
			all_param_vals=all_param_vals_same_cost, # Need to use all_param_vals_same_cost here as we only want to look at a table with just top_k_retvr_values = top_k_retr_vals_base
			fixed_params=RQs[curr_rq]["fixed_params"],
			var_params=RQs[curr_rq]["var_params"],
