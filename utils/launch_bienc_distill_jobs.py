...
def _get_param_config():
	...
	neg_strategy_specific_params = {
		...
		# "top_ce_w_bienc_hard_negs_ml":{
		# 	"distill_n_labels": 50,
		# 	"num_negs": -1, # FIXME: Specify this
		# }
	}
...
def get_graph_eval_job_configs(base_data_dir, base_res_dir, eval_on_train_data):
	...
	for ctr, curr_config in enumerate(all_train_configs):
		...
		embed_type_vals  = ["tfidf", "bienc"]
		max_nbrs_vals  = [5, 10, 20, 50]
		beamsize_vals  = [1, 2, 5, 10]
		entry_method_vals  = ["random", "tfidf", "bienc"]
		
		for embed_type, max_nbrs, beamsize, entry_method in \
			itertools.product(embed_type_vals, max_nbrs_vals, beamsize_vals, entry_method_vals):
			
			if entry_method != "random" and entry_method != embed_type: continue
			
			result_dir = f"{base_res_dir}/models/{data_name}/nm_train={nm_train}/split_idx={split_idx}/{eval_method}/emb={embed_type}_nbr={max_nbrs}_b={beamsize}_entry={entry_method}"
			
			# There are multiple train configs that train on the exact same data so avoid running eval multiple times on the same test data
			if result_dir in processed_res_dirs: continue
			processed_res_dirs[result_dir] = 1
			eval_config = {
				"data_name": data_name,
...
def launch_eval_jobs():
	...
	elif arg_eval_method in ["tfidf", "bm25"]:
		eval_method = arg_eval_method
	...
	elif arg_eval_method == "graph":
		eval_method = arg_eval_method
		all_eval_configs, _, _ = get_graph_eval_job_configs(base_data_dir=base_data_dir, base_res_dir=base_res_dir, eval_on_train_data=eval_on_train_data)
