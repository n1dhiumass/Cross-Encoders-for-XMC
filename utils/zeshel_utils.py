...
def get_dataset_info(data_dir, res_dir, worlds, n_ment=100):
	...
	if res_dir is not None:
		...
		for domain in domains:
			filename 		=  f"{res_dir}/{domain}/ment_to_ent_scores_n_m_{n_ments[domain]}_n_e_{n_ents[domain]}.pkl"
			filename_w_suff =  f"{res_dir}/{domain}/ment_to_ent_scores_n_m_{n_ments[domain]}_n_e_{n_ents[domain]}_all_layers_False.pkl"
			# DATASETS[domain]["crossenc_ment_to_ent_scores"] =  filename_w_suff if os.path.isfile(filename_w_suff) else filename
