	...
	try:
		...
		if rq_name == "RQ1_Model_Performance_At_Equal_Num_Retrieved":
			...
			model_plot_params = {
				...
				"model=CUR~ckpt=None~anc_n_e=50~graph_config=None" : {"name": r"\textsc{CUR-50}"},
				"model=CUR~ckpt=None~anc_n_e=100~graph_config=None" : {"name": r"\textsc{CUR-100}"},
				"model=CUR~ckpt=None~anc_n_e=200~graph_config=None" : {"name": r"\textsc{CUR-200}"},
			}
			model_name_to_plots_params = {
				...
				r"\textsc{CUR-50}" : {"color": "yellowgreen"},
				r"\textsc{CUR-100}" : {"color": "limegreen"},
				r"\textsc{CUR-200}" : {"color": "darkgreen"},
			}
		...
		if curr_x_axis_metric == "cost":
			plt.legend(bbox_to_anchor=(0.4, 1.15), loc='lower left', ncol=len(handles), fontsize=18, bbox_transform=axes[0].transAxes, handletextpad=0.5, columnspacing=1)
			axes[0].xaxis.set_label_coords(2, -0.15)
		elif curr_x_axis_metric == "top_k_retvr":
			plt.legend(bbox_to_anchor=(-0.28, 1.15), loc='lower left', ncol=len(handles), fontsize=18, bbox_transform=axes[0].transAxes, handletextpad=0.5, columnspacing=1)
			axes[0].xaxis.set_label_coords(2, -0.15)
		else:
	...
	try:
		...
		if rq_name == "RQ5_Model_Performance_At_Equal_Test_Cost_CE_Baselines":
			...
			model_plot_params = {
				...
				"model=Fixed_Anc_Ent_CUR~ckpt=None~anc_n_e=100~graph_config=None" : {"name": r"\textsc{itemCUR-100}"},
				"model=Fixed_Anc_Ent_CUR~ckpt=None~anc_n_e=200~graph_config=None" : {"name": r"\textsc{itemCUR-200}"},
			...
			model_name_to_plots_params = {
				...
				r"\textsc{itemCUR-100}" : {"color": "turquoise"},
				r"\textsc{itemCUR-200}" : {"color": "teal"},
				r"\textsc{CUR-100}" : {"color": "yellowgreen"},
				r"\textsc{CUR-200}"	 : {"color": "forestgreen"},
			}
		else:
			raise NotImplementedError(f"RQ = {rq_name} not supported")
...
def plot_rq_4_performance_vs_train_data_size(res_dir, topk, cost, domain_vals, eval_data):
	...
	try:
		...
		fig, axes = plt.subplots(1, len(domain_vals), figsize=(4*len(domain_vals), 4), sharey=True )
		
		width = 0.2
		for domain_ctr, domain in enumerate(sorted(domain_vals)):
			nm_train_vals = [100, 500, 2000] if domain != "pro_wrestling" else [100, 500, 1000]
		...
		# axes[0].set_xlabel("Size of training data", fontsize=24)
		handles, labels = axes[0].get_legend_handles_labels()
...
def plot_heat_map(val_matrix, row_vals, col_vals, out_fname, metric):
	...
	try:
		...
		for cmap_str, cmaps in [("Blues", cm.Blues)]:
			...
			ax.set_xlabel("Number of anchor items", fontsize=28)
			ax.set_ylabel("Number of anchor queries", fontsize=28)
			fig.tight_layout()
...
def plot_rq_0_score_distribution(out_dir, bins):
	...
	# 				 hist = True, kde = True,
	# 				 kde_kws = {'shade': True, 'linewidth': 2, 'clip':(-15, 15)},
	# 				 bins = bins,
